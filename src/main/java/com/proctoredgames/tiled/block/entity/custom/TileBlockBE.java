...
public class TileBlockBE extends BlockEntity {
    ...
    protected void addComponents(ComponentMap.Builder componentMapBuilder) {
        ...
        if (isUniform()) {
            componentMapBuilder.add(ModDataComponentTypes.TILE_BLOCK_TILES, getFace(Direction.DOWN));
        } else {
            componentMapBuilder.add(ModDataComponentTypes.TILE_BLOCK_FACE_TILES,
                    new TilesPerFace(Map.copyOf(this.faceTiles)));
        }
    }
    ...
    protected void readComponents(BlockEntity.ComponentsAccess components) {
        ...
        this.faceTiles.clear();
        TilesPerFace perFace = components.get(ModDataComponentTypes.TILE_BLOCK_FACE_TILES);
        if (perFace != null) {
            this.faceTiles.putAll(perFace.faces());
        } else {
            putAllFaces(components.getOrDefault(ModDataComponentTypes.TILE_BLOCK_TILES, Tiles.DEFAULT));
        }
    }
